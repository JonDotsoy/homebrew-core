import { readableStreamToArrayBuffer, readableStreamToJSON, readableStreamToText } from "bun";
...
export class GhApi {
  ...
  run<T extends string[]>(...args: T) {
    const cmd = [this.bin, ...(this.cache ? ["--cache", this.cache] : []), ...args];
    ...
    return {
      ...
      json: async <R = A<T>>(): Promise<R> => await readableStreamToJSON(subProcess.stdout),
