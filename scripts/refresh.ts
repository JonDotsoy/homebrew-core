...
const release = await ghApi
  .run("api", `/repos/${repoName}/releases/tags/${tagVersion}`)
  .json();
...
const hexFromArrayBuffer = (arrayBuffer: ArrayBuffer) =>
  Array.from(new Uint8Array(arrayBuffer), (e) =>
    e.toString(16).padEnd(2, "0"),
  ).join("");

const renderHash = hexFromArrayBuffer(
  await crypto.subtle.digest("sha1", new TextEncoder().encode(render)),
);
const outputHash = hexFromArrayBuffer(
  await crypto.subtle.digest("sha1", await Bun.file(output).arrayBuffer()),
);
